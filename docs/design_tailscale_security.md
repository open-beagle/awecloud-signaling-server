...
**限制原因**：Headscale 不支持按 User 分配不同网段，所有节点从同一网段顺序分配。

```
实际网段: 100.64.0.0/10

┌─────────────────────────────────────────────────────────────────┐
│ 所有节点共用网段: 100.64.0.0/10                                  │
│                                                                 │
│   Headscale 配置: prefixes.v4: 100.64.0.0/10                    │
│   分配方式: allocation: sequential (顺序分配)                    │
│                                                                 │
│   实际分配示例:                                                  │
│   100.64.0.1   - Agent-A (先注册)                               │
│   100.64.0.2   - Agent-B                                        │
│   100.64.6.4   - Desktop-张三 (后注册，IP 不连续)                │
│   100.64.6.5   - Desktop-李四                                   │
│   100.64.7.1   - Agent-C                                        │
│   ...                                                           │
│                                                                 │
│   ⚠️ Agent 和 Desktop 的 IP 交错分布，无法通过网段区分           │
└─────────────────────────────────────────────────────────────────┘
...
│ 网段             │ 用途                        │ 说明     │
├──────────────────┼─────────────────────────────┼──────────┤
│ 100.64.0.0/10    │ Agent + Desktop 共用        │ 顺序分配 │
└──────────────────┴─────────────────────────────┴──────────┘
```

### 8.3 权限隔离替代方案

由于无法通过网段区分节点类型，采用以下替代方案：

| 隔离方式       | 说明                                           |
| -------------- | ---------------------------------------------- |
| Headscale User | Agent 用 `agent-xxx`，Desktop 用 `desktop-xxx` |
| 数据库记录     | 通过 agents/clients 表区分节点类型             |
| ACL 规则       | 基于具体 IP 地址生成规则，而非网段             |
...
{
  "acls": [
    // Desktop 访问 Agent (基于具体 IP)
    {
      "action": "accept",
      "src": ["100.64.6.4"], // Desktop-张三 (实际 IP)
      "dst": ["100.64.0.1:3306"] // Agent-A:MySQL
    }
  ]
}
```
...
| 决策            | 理想设计             | 实际实现           | 原因                         |
| --------------- | -------------------- | ------------------ | ---------------------------- |
| Agent 固定 IP   | 100.64.x.x 网段      | 共用网段，顺序分配 | Headscale 限制               |
| Desktop 固定 IP | 100.65.x.x 网段      | 共用网段，顺序分配 | Headscale 限制               |
| 网段隔离        | 按网段区分节点类型   | 按 User/数据库区分 | Headscale 不支持按 User 分配 |
| Agent 分组      | 同组互访，跨组需授权 | ✅ 已实现          | ACL 规则控制                 |
| Desktop 多实例  | 同一员工多设备       | ✅ 已实现          | 设备指纹识别                 |
| 非 Ephemeral    | Desktop 持久化节点   | ✅ 已实现          | 保持固定 IP                  |
...
| 问题            | 解决方案              | 状态        |
| --------------- | --------------------- | ----------- |
| Agent 互访      | 分组管理 + ACL        | ✅ 可实现   |
| Desktop 越权    | 动态 ACL + 固定 IP    | ✅ 可实现   |
| 权限撤销无效    | ACL 同步删除          | ✅ 可实现   |
| 外部 Agent 风险 | 无分组隔离 + 端口限制 | ✅ 可实现   |
| Desktop 多实例  | 设备指纹 + 固定 IP    | ✅ 可实现   |
| 网段隔离        | ~~按网段区分~~        | ❌ 无法实现 |

**核心原则**：

- Server 应用层权限 + Headscale 网络层 ACL = 双重保障
- ~~Agent (100.64.x.x) 和 Desktop (100.65.x.x) 网段隔离~~ → 实际共用网段，通过 User/数据库区分
- 固定 IP 保证服务地址稳定
- 分组管理防止横向移动
...
- 不支持按 User 分配不同网段
- 所有节点从 `100.64.0.0/10` 顺序分配
- 需通过 ACL 规则和数据库记录实现权限隔离
