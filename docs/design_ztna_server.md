...
| 服务管理 | ProxyService（手动配置）  | AgentSVC 自动发现替代，ProxyService 逐步废弃  |
| 权限管理 | SSH ACL + 服务公开/私有   | 新增 K8S ACL + Endpoint 跳跃 ACL              |
| Endpoint | 无                        | 新增 Endpoint 数据模型和管理 API              |
| 域名管理 | 无                        | 域名注册表，.k8s 后缀映射                     |
| 审计日志 | 连接审计                  | 操作级审计（SSH/K8S/SVC 直连和跳跃）          |
...
Agent 本机：
  ...
  api.<agent-name>.k8s:6443                    → AgentK8S
  <service>.<namespace>.<agent-name>.k8s       → AgentSVC

Endpoint 跳跃：
  <endpoint>.<agent-name>.k8s:22               → EndpointSSH
  api.<endpoint>.<agent-name>.k8s:6443         → EndpointK8S
  <service>.<namespace>.<endpoint>.<agent-name>.k8s → EndpointSVC
```
...
| 模型                      | 层级    | 说明                         |
| ------------------------- | ------- | ---------------------------- |
| AclK8sUserPermission      | 第 3 层 | AgentK8S 权限（用户级）      |
| AclK8sGroupPermission     | 第 3 层 | AgentK8S 权限（分组级）      |
| AclSSHJumpUserPermission  | 第 4 层 | EndpointSSH 跳跃授权（用户） |
| AclSSHJumpGroupPermission | 第 4 层 | EndpointSSH 跳跃授权（分组） |
| AclK8sJumpUserPermission  | 第 4 层 | EndpointK8S 跳跃授权（用户） |
| AclK8sJumpGroupPermission | 第 4 层 | EndpointK8S 跳跃授权（分组） |
| AclSVCJumpUserPermission  | 第 4 层 | EndpointSVC 跳跃授权（用户） |
| AclSVCJumpGroupPermission | 第 4 层 | EndpointSVC 跳跃授权（分组） |
...
| 表名                          | 说明                         |
| ----------------------------- | ---------------------------- |
| acl_k8s_user_permission       | AgentK8S 权限（用户级）      |
| acl_k8s_group_permission      | AgentK8S 权限（分组级）      |
| acl_ssh_jump_user_permission  | EndpointSSH 跳跃授权（用户） |
| acl_ssh_jump_group_permission | EndpointSSH 跳跃授权（分组） |
| acl_k8s_jump_user_permission  | EndpointK8S 跳跃授权（用户） |
| acl_k8s_jump_group_permission | EndpointK8S 跳跃授权（分组） |
| acl_svc_jump_user_permission  | EndpointSVC 跳跃授权（用户） |
| acl_svc_jump_group_permission | EndpointSVC 跳跃授权（分组） |
...
ProxyService（手动配置端口映射）将被 AgentSVC（自动发现）逐步替代。迁移策略待定：可以保留兼容（手动 + 自动共存），也可以统一到 AgentSVC。
...
操作类型：
  ...
  k8s_direct    → K8S API 直连 Agent（第 3 层）
  svc_direct    → SVC 直连 Agent（AgentSVC，第 1 层）
  ssh_jump      → 通过 Agent 跳跃到 EndpointSSH（第 4 层）
  k8s_jump      → 通过 Agent 跳跃到 EndpointK8S（第 4 层）
  svc_jump      → 通过 Agent 跳跃到 EndpointSVC（第 4 层）
```
