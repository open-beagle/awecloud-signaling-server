...
func (d *K8SServiceDiscovery) initK8SConnection() error {
	...
	// 优先读取 Service Account Token（Pod 内部署）
	...
	// 降级：从 kubeconfig 提取 Token（物理节点部署）
	logger.Infof("Service Account Token 不可用，降级到 kubeconfig: %v", err)
	...
	if kubeconfigPath != "" {
		if token := extractTokenFromKubeconfig(kubeconfigPath); token != "" {
			d.k8sToken = token
			logger.Infof("从 kubeconfig 提取 Token 成功: %s", kubeconfigPath)
		} else {
			logger.Warnf("kubeconfig 中未找到 Token，将使用证书认证或匿名访问")
		}
	}
...
// extractTokenFromKubeconfig 从 kubeconfig 文件中提取 Bearer Token
// kubeconfig 中 user.token 字段即为 Bearer Token（ServiceAccount 或 exec 凭证）
func extractTokenFromKubeconfig(kubeconfigPath string) string {
	data, err := readFile(kubeconfigPath)
	if err != nil {
		return ""
	}
