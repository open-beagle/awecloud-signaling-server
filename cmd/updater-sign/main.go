package main

import (
	"crypto/ed25519"
	"encoding/base64"
	"fmt"
	"os"
	"strings"
)

func main() {
	if len(os.Args) != 2 {
		fatal("usage: updater-sign <lowercase-sha256>")
	}
	digest := strings.TrimSpace(os.Args[1])
	if len(digest) != 64 || digest != strings.ToLower(digest) || strings.Trim(digest, "0123456789abcdef") != "" {
		fatal("digest must be a 64-character lowercase SHA256")
	}
	encodedKey := strings.TrimSpace(os.Getenv("SIGNAL_UPDATER_PRIVATE_KEY"))
	privateKey, err := base64.StdEncoding.DecodeString(encodedKey)
	if err != nil || len(privateKey) != ed25519.PrivateKeySize {
		fatal("SIGNAL_UPDATER_PRIVATE_KEY must be a base64-encoded Ed25519 private key")
	}
	signature := ed25519.Sign(ed25519.PrivateKey(privateKey), []byte(digest))
	fmt.Print(base64.StdEncoding.EncodeToString(signature))
}

func fatal(message string) {
	fmt.Fprintln(os.Stderr, message)
	os.Exit(1)
}
