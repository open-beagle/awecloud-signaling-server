...
func TestNodeRuntimeStoreBasicAndHeartbeat(t *testing.T) {
	...
	// 更新心跳
	now := time.Now()
	rev, err := store.UpdateHeartbeat(10, "100.64.0.10", "my-desktop-renamed", "v1.0.2", "{}", "v1", "v1", now)
	if err != nil {
	...
	// 模拟 Flush 时又来了一次心跳
	_, _ = store.UpdateHeartbeat(10, "100.64.0.10", "my-desktop-renamed", "v1.0.2", "{}", "v1", "v1", now.Add(time.Second))
