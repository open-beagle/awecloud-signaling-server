...
func TestManagementAuthorizationRoleMatrix(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "platform updater sync", admin: admins["platform"].ID, method: http.MethodPost, path: "/api/v1/admin/updater/sync", want: http.StatusNoContent},
		...
		{name: "viewer updater sync denied", admin: admins["viewer"].ID, method: http.MethodPost, path: "/api/v1/admin/updater/sync", want: http.StatusForbidden},
