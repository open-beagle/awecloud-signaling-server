...
func prepareProviderSupplyAPIFixture(t *testing.T, flags config.FeatureFlagsSection) (managementContextAPIFixture, LoginResponse) {
	...
	provider.PATCH("/resources/:id/host-domain-label", RequireManagementPermission(service.PermissionProviderResourcesWrite), RequireFeatureFlag(flags, config.FeatureResourceModelWrite, true), RequireIfMatch(), providerAPI.UpdatePlatformHostDomainLabel)
...
func TestProviderSupplyAPICreateIsIdempotentAndTransactional(t *testing.T) {
	fixture, login := prepareProviderSupplyAPIFixture(t, config.FeatureFlagsSection{ResourceModelWrite: true})
	body := `{"type":"agent","stable_key":"agent-api-a","runtime_name":"agent-api-a","domain_label":"agent-api-a","credential_revision":1,"reason":"register API agent"}`
	...
	replayed := providerAPIRequest(fixture, login, http.MethodPost, providerTechnicalResourceCreateRoute, fixture.provider.ID,
		`{"reason":"register API agent","credential_revision":1,"domain_label":"agent-api-a","runtime_name":"agent-api-a","stable_key":"agent-api-a","type":"agent"}`, headers)
	...
	reused := providerAPIRequest(fixture, login, http.MethodPost, providerTechnicalResourceCreateRoute, fixture.provider.ID,
		`{"type":"agent","stable_key":"agent-api-other","runtime_name":"agent-api-other","domain_label":"agent-api-other","credential_revision":1,"reason":"different request"}`, headers)
...
func TestProviderSupplyAPIAuditFailureRollsBackMutationAndOutbox(t *testing.T) {
	...
	response := providerAPIRequest(fixture, login, http.MethodPost, providerTechnicalResourceCreateRoute, fixture.provider.ID,
		`{"type":"agent","stable_key":"agent-audit-failure","runtime_name":"agent-audit-failure","domain_label":"agent-audit-failure","credential_revision":1,"reason":"verify atomic rollback"}`,
...
func TestProviderSupplyAPIAcceptCandidateCreatesDraftScopesOnce(t *testing.T) {
	...
	technical := model.TechnicalResource{
		...
		StableKey: "candidate-api-agent", DomainLabel: "candidate-api-agent", LifecycleState: model.TechnicalResourceRegistered,
...
func TestProviderSupplyAPIWriteFlagAndCreateHeadersFailClosed(t *testing.T) {
	t.Run("resource write flag disabled", func(t *testing.T) {
		fixture, login := prepareProviderSupplyAPIFixture(t, config.FeatureFlagsSection{})
		response := providerAPIRequest(fixture, login, http.MethodPost, providerTechnicalResourceCreateRoute, fixture.provider.ID,
			`{"type":"agent","stable_key":"disabled-agent","runtime_name":"disabled-agent","domain_label":"disabled-agent","credential_revision":1,"reason":"must stay disabled"}`, nil)
		require.Equal(t, http.StatusServiceUnavailable, response.Code, response.Body.String())
