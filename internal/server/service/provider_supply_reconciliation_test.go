...
func TestProviderSupplyCompleteSnapshotReconcilesNamespaceLeaseAndScope(t *testing.T) {
	...
	require.Zero(t, result.UpdatedResources)
	require.NoError(t, fixture.database.First(&missingObservation, "id = ?", missingObservation.ID).Error)
	...
	require.Equal(t, model.ResourceScopeAllocatable, missingScope.LifecycleState)
	...
	require.Equal(t, 1, resource.AllocatableScopeCount)
	...
	require.GreaterOrEqual(t, outboxCount, int64(2))
