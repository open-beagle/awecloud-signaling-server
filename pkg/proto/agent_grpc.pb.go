...
const (
	...
	AgentService_ReportWorkloadInventory_FullMethodName = "/awecloud.signaling.AgentService/ReportWorkloadInventory"
...
type AgentServiceClient interface {
	...
	// 上报 Namespace 内工作负载清单；复用既有 Deploy Token 和可信 Binding。
...
func (c *agentServiceClient) ReportWorkloadInventory(ctx context.Context, opts ...grpc.CallOption) (grpc.BidiStreamingClient[WorkloadInventoryEnvelope, WorkloadInventoryAck], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &AgentService_ServiceDesc.Streams[2], AgentService_ReportWorkloadInventory_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
...
// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type AgentService_ReportWorkloadInventoryClient = grpc.BidiStreamingClient[WorkloadInventoryEnvelope, WorkloadInventoryAck]
...
func (c *agentServiceClient) SVCProxy(ctx context.Context, opts ...grpc.CallOption) (grpc.BidiStreamingClient[SVCProxyData, SVCProxyData], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &AgentService_ServiceDesc.Streams[3], AgentService_SVCProxy_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
...
func (UnimplementedAgentServiceServer) ReportWorkloadInventory(grpc.BidiStreamingServer[WorkloadInventoryEnvelope, WorkloadInventoryAck]) error {
...
func _AgentService_ReportWorkloadInventory_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(AgentServiceServer).ReportWorkloadInventory(&grpc.GenericServerStream[WorkloadInventoryEnvelope, WorkloadInventoryAck]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type AgentService_ReportWorkloadInventoryServer = grpc.BidiStreamingServer[WorkloadInventoryEnvelope, WorkloadInventoryAck]
...
var AgentService_ServiceDesc = grpc.ServiceDesc{
	...
	Streams: []grpc.StreamDesc{
		...
		},
		{
			...
			Handler:       _AgentService_ReportWorkloadInventory_Handler,
			ServerStreams: true,
			ClientStreams: true,
		},
		{
