<template>
  <div class="tenant-page">
    <PageHeader title="资源目录" description="查看当前租户 Allocation 内发布的 ContainerSSH 与 ContainerService。运行地址和底层端口不作为授权标识。">
    ...
    <section class="surface">
      <div class="toolbar">
        ...
        <el-select v-model="filters.type" class="filter-select" clearable placeholder="全部类型" @change="applyFilters"><el-option label="ContainerSSH" value="container_ssh" /><el-option label="ContainerService" value="container_service" /></el-select>
      ...
      <el-table v-loading="loading" :data="items" stripe>
        ...
        <el-table-column label="Namespace" min-width="190"><template #default="{ row }"><span>{{ row.namespace_name || '-' }}</span><span class="secondary mono">{{ row.namespace_scope_id || '-' }}</span></template></el-table-column>
...
const typeLabel = (value: string) => value === 'container_ssh' ? 'ContainerSSH' : value === 'container_service' ? 'ContainerService' : value
...
const targetLabel = (row: TenantResourceV2) => row.type === 'container_service' ? `${row.service_name || '-'}:${row.port_number || '-'}` : `${row.pod_name || row.workload_name || '-'} / ${row.container_name || '-'}`
