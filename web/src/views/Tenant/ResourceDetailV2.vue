  ...
  <div class="detail-page" v-loading="loading">
    ...
    <el-dialog v-model="grantDialogVisible" title="添加资源授权" width="520px">
      <el-form label-position="top">
        ...
        <el-form-item label="Action"><el-input :model-value="grantActionLabel" disabled /></el-form-item>
...
import { createTenantGrantV2, getTenantGrantsV2, getTenantResourceV2, type TenantGrantV2, type TenantResourceV2 } from '@/api/tenantResourcesV2'
...
const grantForm=reactive<{subject_type:'user'|'group';subject_user_id?:number;subject_group_id?:number;expires_at?:string}>({subject_type:'user'})
const canGrant=computed(()=>!!resource.value&&tenantStore.canTenant('tenant.grants.write')&&(resource.value.type==='container_ssh'||resource.value.type==='container_service'))
const grantAction=computed<'shell'|'connect'>(()=>resource.value?.type==='container_service'?'connect':'shell')
const grantActionLabel=computed(()=>grantAction.value==='connect'?'Connect':'Shell')
...
const createGrant=async()=>{if(!resource.value)return;if(grantForm.subject_type==='user'&&!grantForm.subject_user_id){ElMessage.warning('请选择客户成员');return}if(grantForm.subject_type==='group'&&!grantForm.subject_group_id){ElMessage.warning('请选择客户用户组');return}creatingGrant.value=true;try{const subject=grantForm.subject_type==='user'?{type:'user' as const,user_id:grantForm.subject_user_id}:{type:'group' as const,group_id:grantForm.subject_group_id};const response=await createTenantGrantV2(tenantStore.tenantId,{resource_id:resource.value.resource_id,subject,actions:[grantAction.value],expires_at:grantForm.expires_at,max_session_seconds:28800});if(response.success){ElMessage.success('访问授权已创建');grantDialogVisible.value=false;grantForm.subject_user_id=undefined;grantForm.subject_group_id=undefined;await loadGrants()}}finally{creatingGrant.value=false}}
