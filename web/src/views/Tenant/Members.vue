  ...
  <div class="page-container">
    ...
    <el-table v-loading="loading" :data="members" empty-text="当前租户还没有成员">
      ...
      <el-table-column label="状态" width="120"><template #default="scope"><el-tag size="small" effect="plain" :type="scope.row.enabled ? 'success' : 'info'">{{ scope.row.enabled ? '有效' : '已停用' }}</el-tag></template></el-table-column>
		...
		<el-table-column v-if="tenantStore.canTenant('tenant.members.write')" label="操作" width="100" fixed="right"><template #default="scope"><el-button v-if="scope.row.enabled" type="danger" link :loading="disabling === scope.row.user_id" @click="disableMember(scope.row)">停用</el-button><span v-else class="secondary">不可操作</span></template></el-table-column>
    </el-table>
  </div>
</template>
...
import { disableTenantMember, getTenantMembers, type TenantMember } from '@/api/resource'
