...
const handleView = (row: K8SUnifiedACLItem) => {
  if (row.type === 'agent') {
    // Agent 类型 → 跳转到 Agent K8S 授权详情页
    router.push({ path: `/acl/k8s/${row.id}`, query: { name: row.name } })
  } else {
    // Endpoint 类型 → 跳转到 Endpoint K8SAPI 授权详情页
    router.push({ path: `/acl/endpoint-k8sapi/${row.id}`, query: { name: row.name } })
  }
}
