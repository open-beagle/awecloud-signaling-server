<template>
  <div class="acl-group-list">
    <!-- 搜索 -->
    <el-card class="search-card" shadow="never">
      <el-form :inline="true" :model="searchForm" class="search-form">
        <el-form-item :label="$t('common.search')">
          <el-input v-model="searchForm.search" :placeholder="$t('acl.searchGroupPlaceholder')" clearable style="width: 240px" @keyup.enter="handleSearch" />
        </el-form-item>
        <el-form-item>
    ...
    <el-card shadow="never">
      <el-table v-loading="loading" :data="groups" stripe>
        ...
        <el-table-column prop="alias" :label="$t('group.description')" min-width="150" />
        <el-table-column prop="member_count" :label="$t('acl.memberCount')" width="100" align="center">
          <template #default="{ row }">
            <el-tag type="info" size="small">{{ row.member_count }}</el-tag>
          </template>
        </el-table-column>
...
import { getGroupACLList, type GroupACLItem } from '@/api/acl'
import { formatTime } from '@/utils/time'

const router = useRouter()

const loading = ref(false)
const groups = ref<GroupACLItem[]>([])
...
const fetchGroups = async () => {
  loading.value = true
  try {
    const res = await getGroupACLList({
