<template>
  <div class="acl-endpoint-k8s-service-list">
...
import { getEndpointK8SServiceACLList, type EndpointK8SServiceACLItem } from '@/api/aclEndpointK8sservice'
import { formatTime } from '@/utils/time'

const router = useRouter()
const loading = ref(false)
const list = ref<EndpointK8SServiceACLItem[]>([])
...
const fetchList = async () => {
  loading.value = true
  try {
    const res = await getEndpointK8SServiceACLList({ search: searchForm.search || undefined, page: pagination.page, size: pagination.size })
...
const handleView = (row: EndpointK8SServiceACLItem) => { router.push({ path: `/acl/endpoint-k8sservice/${row.id}`, query: { name: row.name } }) }

onMounted(() => { fetchList() })
</script>

<style scoped>
.acl-endpoint-k8s-service-list { width: 100%; }
