    ...
    <el-card class="search-card" shadow="never">
      <el-form :inline="true" :model="searchForm" class="search-form">
        <el-form-item :label="$t('common.search')">
          <el-input v-model="searchForm.search" :placeholder="$t('acl.searchUserPlaceholder')" clearable style="width: 240px" @keyup.enter="handleSearch" />
        </el-form-item>
        <el-form-item>
...
import { getUserACLList, type UserACLItem } from '@/api/acl'
import { formatTime } from '@/utils/time'

const router = useRouter()

const loading = ref(false)
const users = ref<UserACLItem[]>([])
...
const fetchUsers = async () => {
  loading.value = true
  try {
    const res = await getUserACLList({
...
// 查看详情
const handleView = (row: UserACLItem) => {
  router.push({ path: `/acl/users/${row.id}`, query: { name: row.name } })
}
