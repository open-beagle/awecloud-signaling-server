<template>
  <el-dialog v-model="visible" :title="$t('acl.addGroupAuth')" width="500px" @close="handleClose">
...
import { addServiceACLGroups, addUserACLGroups, addGroupACLGroups } from '@/api/acl'
...
const handleSubmit = async () => {
  ...
  await formRef.value.validate(async (valid) => {
    ...
    try {
      let res
      if (props.serviceId) {
        res = await addServiceACLGroups(props.serviceId, form.groupIds)
      } else if (props.type === 'user' && props.targetId) {
        res = await addUserACLGroups(props.targetId, form.groupIds)
      } else if (props.type === 'group' && props.targetId) {
        res = await addGroupACLGroups(props.targetId, form.groupIds)
      }
