<template>
  <el-dialog v-model="visible" :title="$t('acl.addK8SServiceUserAuth')" width="560px" @close="handleClose">
...
import { addK8SServiceACLUsers } from '@/api/aclK8sService'
...
const form = reactive({ userIds: [] as number[], namespaces: ['*'] as string[], serviceNames: ['*'] as string[] })
...
const handleSubmit = async () => {
  ...
  await formRef.value.validate(async (valid) => {
    ...
    try {
      const res = await addK8SServiceACLUsers(props.agentId, { user_ids: form.userIds, namespaces: form.namespaces, service_names: form.serviceNames })
...
const handleClose = () => { form.userIds = []; form.namespaces = ['*']; form.serviceNames = ['*']; formRef.value?.resetFields(); visible.value = false }
