<template>
  <el-dialog v-model="visible" :title="$t('acl.addSSHUserAuth')" width="500px" @close="handleClose">
...
import { addSSHACLUsers } from '@/api/acl'
...
const availableUsers = computed(() => {
  return users.value.filter(u => u.role === 'client')
})
...
const handleSubmit = async () => {
  ...
  await formRef.value.validate(async (valid) => {
    ...
    try {
      const res = await addSSHACLUsers(props.agentId, form.userIds, form.sshUsers)
