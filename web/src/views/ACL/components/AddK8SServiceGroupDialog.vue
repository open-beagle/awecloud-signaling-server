<template>
  <el-dialog v-model="visible" :title="$t('acl.addK8SServiceGroupAuth')" width="560px" @close="handleClose">
    <el-form ref="formRef" :model="form" :rules="rules" label-width="120px">
      ...
      <el-form-item :label="$t('acl.serviceNames')" prop="serviceNames">
        <el-select v-model="form.serviceNames" multiple filterable allow-create default-first-option :placeholder="$t('acl.serviceNamesPlaceholder')" style="width: 100%">
          <el-option label="* (全部)" value="*" />
        </el-select>
      </el-form-item>
    </el-form>
...
import { addK8SServiceACLGroups } from '@/api/aclK8sService'
...
const form = reactive({ groupIds: [] as number[], namespaces: ['*'] as string[], serviceNames: ['*'] as string[] })
...
const handleSubmit = async () => {
  ...
  await formRef.value.validate(async (valid) => {
    ...
    try {
      const res = await addK8SServiceACLGroups(props.agentId, { group_ids: form.groupIds, namespaces: form.namespaces, service_names: form.serviceNames })
...
const handleClose = () => { form.groupIds = []; form.namespaces = ['*']; form.serviceNames = ['*']; formRef.value?.resetFields(); visible.value = false }
