<template>
  <div class="acl-endpoint-ssh-list">
    <!-- 搜索 -->
    ...
    <el-card shadow="never">
      <el-table v-loading="loading" :data="list" stripe>
        ...
        <el-table-column prop="host" :label="$t('endpoint.host')" min-width="120" />
...
import { getEndpointSSHACLList, type EndpointSSHACLItem } from '@/api/aclEndpointSsh'
import { formatTime } from '@/utils/time'

const router = useRouter()
const loading = ref(false)
const list = ref<EndpointSSHACLItem[]>([])
...
const fetchList = async () => {
  loading.value = true
  try {
    const res = await getEndpointSSHACLList({ search: searchForm.search || undefined, page: pagination.page, size: pagination.size })
...
const handleView = (row: EndpointSSHACLItem) => { router.push({ path: `/acl/endpoint-ssh/${row.id}`, query: { name: row.name } }) }

onMounted(() => { fetchList() })
</script>

<style scoped>
.acl-endpoint-ssh-list { width: 100%; }
