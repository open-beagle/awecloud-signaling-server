<template>
  <div class="acl-k8s-service-unified-list">
    <!-- 搜索 -->
...
import { getK8SServiceUnifiedACLList, type K8SServiceUnifiedACLItem } from '@/api/aclK8sServiceUnified'
import { formatTime } from '@/utils/time'

const router = useRouter()
const loading = ref(false)
const list = ref<K8SServiceUnifiedACLItem[]>([])
...
const fetchList = async () => {
  loading.value = true
  try {
    const res = await getK8SServiceUnifiedACLList({
    ...
    console.error('获取 K8S Service 授权合并列表失败:', error)
  } finally {
    loading.value = false
  }
}
...
<style scoped>
.acl-k8s-service-unified-list { width: 100%; }
