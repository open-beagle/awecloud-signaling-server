  ...
  <div class="endpoint-list">
    ...
    <el-card shadow="never">
      <el-table v-loading="loading" :data="list" stripe>
        ...
        <el-table-column prop="api_server" :label="$t('endpoint.apiServer')" min-width="200">
          <template #default="{ row }">{{ row.api_server || $t('endpoint.autoDetect') }}</template>
        </el-table-column>
    ...
    <!-- 创建/编辑弹窗 -->
    <el-dialog v-model="showCreateDialog" :title="editingItem ? $t('endpoint.editK8SAPI') : $t('endpoint.createK8SAPI')" width="500px" @closed="resetForm">
      <el-form ref="formRef" :model="form" :rules="rules" label-width="120px">
        ...
        <el-form-item :label="$t('endpoint.apiServer')">
          <el-input v-model="form.api_server" :placeholder="$t('endpoint.apiServerPlaceholder')" />
        </el-form-item>
        <el-form-item :label="$t('endpoint.kubeconfigRef')">
          <el-input v-model="form.kubeconfig_ref" :placeholder="$t('endpoint.kubeconfigRefPlaceholder')" />
        </el-form-item>
      </el-form>
      <template #footer>
...
import { getEndpoints, createEndpoint, updateEndpoint, deleteEndpoint, type EndpointK8SAPI } from '@/api/endpoint'
...
const list = ref<EndpointK8SAPI[]>([])
const agents = ref<{ id: number; name: string; alias?: string }[]>([])
const searchForm = reactive({ search: '' })
const pagination = reactive({ page: 1, size: 20, total: 0 })
const showCreateDialog = ref(false)
const editingItem = ref<EndpointK8SAPI | null>(null)
const formRef = ref<FormInstance>()

const form = reactive({ user_id: undefined as number | undefined, name: '', alias: '', api_server: '', kubeconfig_ref: '' })
const rules = {
...
const fetchList = async () => {
  loading.value = true
  try {
    const res = await getEndpoints('k8sapi', { search: searchForm.search || undefined, page: pagination.page, size: pagination.size })
    if (res.success && res.data) { list.value = res.data; pagination.total = res.total }
  } catch (e) { console.error('获取 K8SAPI Endpoint 列表失败:', e) } finally { loading.value = false }
}
...
const handleEdit = (row: EndpointK8SAPI) => {
  editingItem.value = row
  form.user_id = row.user_id; form.name = row.name; form.alias = row.alias || ''; form.api_server = row.api_server; form.kubeconfig_ref = row.kubeconfig_ref
  showCreateDialog.value = true
}

const resetForm = () => {
  editingItem.value = null
  form.user_id = undefined; form.name = ''; form.alias = ''; form.api_server = ''; form.kubeconfig_ref = ''
...
const handleSubmit = async () => {
  ...
  try {
    ...
    const res = editingItem.value
      ? await updateEndpoint('k8sapi', editingItem.value.id, data)
      : await createEndpoint('k8sapi', data)
    if (res.success) {
...
const handleDelete = async (row: EndpointK8SAPI) => {
  try {
    const res = await deleteEndpoint('k8sapi', row.id)
