<template>
  <div class="provider-page">
    <PageHeader title="管理员" description="查看当前资源方的管理成员、角色、生效区间和权限版本。此页面不授予 Tenant 访问权限。">
    ...
    <section class="data-surface">
      <div class="toolbar">
        ...
        <el-select v-model="filters.role" class="filter-select" clearable placeholder="全部角色" @change="applyFilters">
          <el-option label="资源方管理员" value="provider_admin" />
          <el-option label="资源运营员" value="provider_operator" />
          <el-option label="资源观察员" value="provider_viewer" />
        </el-select>
      ...
      <el-table v-loading="loading" :data="items" stripe>
        ...
        <el-table-column label="角色" width="140"><template #default="{ row }">{{ roleLabel(row.role) }}</template></el-table-column>
      ...
      <el-empty v-if="!loading && !errorMessage && items.length === 0" description="当前资源方没有符合条件的管理成员" />
...
import { getProviderMemberships, type ProviderManagementRole, type ProviderMembership } from '@/api/providerSupply'
import { useWorkspaceStore } from '@/stores/workspace'

const workspaceStore = useWorkspaceStore()
const loading = ref(false)
const errorMessage = ref('')
const items = ref<ProviderMembership[]>([])
...
const load = async () => {
  ...
  errorMessage.value = ''
  try {
    const response = await getProviderMemberships(providerId, {
...
const roleLabel = (role: ProviderManagementRole) => ({ provider_admin: '资源方管理员', provider_operator: '资源运营员', provider_viewer: '资源观察员' }[role])
const membershipState = (item: ProviderMembership) => {
  const now = Date.now()
...
const stateLabel = (item: ProviderMembership) => ({ active: '生效中', scheduled: '待生效', expired: '已过期', disabled: '已停用' }[membershipState(item)])
const stateTag = (item: ProviderMembership) => ({ active: 'success', scheduled: 'warning', expired: 'info', disabled: 'danger' }[membershipState(item)] as any)
const formatTime = (value?: string) => value ? new Date(value).toLocaleString('zh-CN', { hour12: false }) : '长期有效'

watch(() => workspaceStore.providerId, () => { filters.search = ''; filters.role = ''; filters.state = ''; pagination.page = 1; load() })
onMounted(load)
</script>
